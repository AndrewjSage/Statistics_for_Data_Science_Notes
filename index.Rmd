...
A Washington Post [poll](https://www.washingtonpost.com/context/sept-8-13-2020-washington-post-abc-news-poll-of-minnesota-and-wisconsin/8e5dbd4b-4746-4e45-821e-eb8c8645d5c1/), of 702 registered voters in the state of Wisconsin, taken from September 8-13, 2020 showed that:   
...
Candidate <- c("Biden", "Trump", "Other")
Percentage <- c(48,48,4)
Populationdf <- data.frame(Candidate, Percentage)

Populationdf$Candidate <- factor(Populationdf$Candidate, levels = c("Biden", "Trump", "Other"))

ggplot(data=Populationdf,aes(x=Candidate, y=Percentage)) +
  geom_bar(mapping=aes(x=Candidate, y=Percentage), stat = "identity", fill="lightblue") + 
  geom_text(aes(label=paste(Percentage, "%")), vjust=1.6, color="white", size=3.5)

```



### A sample of 702 voters
...
Candidate <- c("Biden", "Other", "Trump")
Percentage <- round(summary(Sample$Vote)/length(Sample$Vote)*100,1)
Sampledf <- data.frame(Candidate, Percentage)

Sampledf$Candidate <- factor(Sampledf$Candidate, levels = c("Biden", "Trump", "Other"))

ggplot(data=Sampledf,aes(x=Candidate, y=Percentage)) +
  geom_bar(mapping=aes(x=Candidate, y=Percentage), stat = "identity", fill="lightblue") + 
  geom_text(aes(label=paste(Percentage, "%")), vjust=1.6, color="white", size=3.5)
```

### A Second Sample of 702 Voters
...
### Histogram of Difference in Sample Proportions
...
### Sampling Dist. for Diff. in Proportions
...
### Difference in Sample Proportions (cont.)


```{r, echo=FALSE, fig.height=2.5, fig.width=7}
q.025 <- quantile(PollingSim$PropDiff, .025)
q.975 <- quantile(PollingSim$PropDiff, .975)
ggplot(data=PollingSim, aes(x=PropDiff, fill=!(PropDiff>=q.975 | PropDiff<= q.025)))+ xlim(c(-.1, .1)) +  geom_histogram(color="white", boundary = -0.1,boundary = 0.1, binwidth=0.001) + 
  xlab(" Dem. Prop. - Rep. Prop") + ylab("Frequency") +
  ggtitle("Difference in Sample Proportions, assuming Population Proportions are Equal") +     geom_vline(xintercept=c(q.025,q.975), color="red") + 
  annotate("text", label="2.5% of Polls", x= -0.10, y=170) + 
  annotate("text", label="95% of Polls", x= 0, y=170) +
  annotate("text", label="2.5% of Polls", x= 0.09, y=170)+ theme(legend.position = "none")
```
...
* Approximately 95% of the polls were within $\pm 7$ of the actual difference, which in this case was assumed to be 0.

### Conclusions from Election Polling Simulation
...
```{r, echo=FALSE}
PollingSim <- PollingSim %>% mutate(Lower = PropDiff - 0.07, 
                                    Upper = PropDiff + 0.07,
                                    Contains0 = Lower < 0 & Upper > 0 )
```


```{r, echo=FALSE, fig.height=4, fig.width=7}
ggplot(data=PollingSim[1:100,], aes(xmin = Lower, xmax = Upper, y = Sim, color=Contains0))+
  geom_linerange(lwd=1) + 
  geom_point(aes(x=PropDiff, y=Sim)) + 
  labs(x="Dem. Prop - Rep. Prop", title='95% Confidence Intervals') + theme_bw() + 
...
## Bootstrap Sampling for Election Example
...
### Example: Election Polling Survey
...
### Illustration of the Bootstrap "Idea"
...
### Bootstrap via Resampling Illustration
...
1. Take a sample of the same size as our original sample, by sampling cases from the original sample, with replacement. Since are sampling with replacement, some cases will come up multiple times and others not at all in our bootstrap sample. 
...
### Code for Bootstrapping for Difference In Proportions
...
### Bootstrap Dist. for Diff. in Proportion

```{r,  echo=FALSE, fig.height=2.5}
BootstrapDiff_Dist <- ggplot(data=Vote_BootstrapResults, aes(x=Difference)) + geom_histogram(fill="lightblue", color="white") + 
  ggtitle("Bootstrap Distribution for Difference in Proportion") + 
  xlab("Proportion Democratic - Proportion Republican") + ylab("Frequency") 
BootstrapDiff_Dist + geom_vline(xintercept=0.04)
```
...
We can be 95% confident that Biden is between `r round(q.975*100, 1)` percentage points ahead, and `r -1*round(q.025*100, 1)` percentage points behind.   


### 95% Bootstrap Percentile Confidence Interval Visually

```{r, echo=FALSE}
ggplot(data=Vote_BootstrapResults, aes(x=Difference)) +  
  geom_histogram(color="white", fill="lightblue", boundary = -0.1,boundary = 0.1, binwidth=0.01) + 
  xlab(" Biden Prop. - Trump Prop") + ylab("Frequency") +
  ggtitle("Bootstrap Distribution for Difference in Proportion") +   
  geom_vline(xintercept=c(q.025,q.975), color="red") + 
  annotate("text", label=".025 of Area", x= -0.05, y=200) + 
  annotate("text", label=".95 of Area", x= 0.04, y=200) +
  annotate("text", label=".025 of Area", x= 0.13, y=200)
```
...
If large number of polls of 702 voters were taken, the standard deviation in the distribution of the difference in support between Biden and Trump is estimated to be `r sd(Vote_BootstrapResults$Difference)`
...
We can be 95% confident that Biden is between `r round(100*(0.04 + 2*SE), 1)` percentage points ahead, and `r -round(100*(0.04 - 2*SE), 1)` percentage points behind.   
...
### 90% Bootstrap Confidence Interval Visually

```{r, echo=FALSE}
ggplot(data=Vote_BootstrapResults, aes(x=Difference)) + 
  geom_histogram(color="white", boundary = -0.1,boundary = 0.1, binwidth=0.01, fill="lightblue") + 
  xlab(" Biden Prop. - Trump Prop") + ylab("Frequency") +
  ggtitle("Difference in Sample Proportions, assuming Population Proportions are Equal") +  
  geom_vline(xintercept=c(q.05,q.95), color="red") + 
  annotate("text", label=".05 of Area", x= -0.05, y=200) + 
  annotate("text", label=".90 of Area", x= 0.04, y=200) +
  annotate("text", label=".05 of Area", x= 0.125, y=200) + theme(legend.position = "none")
```
...
We are 95% confident that the standard deviation in mercury level is all Florida Lakes is between `r round(Slope-2*SE,2)` and `r round(Slope+2*SE,2)` ppm.  
...
We are 95% confident that the standard deviation in mercury level is all Florida Lakes is between `r round(Int-2*SE,2)` and `r round(Int+2*SE,2)` ppm.  
...
## Standard Error Formulas and Theory-Based Intervals
...
### Standard Error for the Mean (continued)
...
### Standard Error in R Output for Lakes
...
### Standard Error in R Output for Lakes Difference 
...
### Standard Error for Slope and Intercept in SLR (cont.)
...
ICplot <- ggplot(data=Icecream1, aes(x=time, y=amount)) + geom_point() + ggtitle("Icecream Dispensed") + xlab("Time Pressing Dispensor") + ylab("Amount Dispensed") + geom_abline(slope=2, intercept=0, color="red") + 
...
### Scatterplot for Ice Cream Sample Data
...
```{r, fig.height=4, fig.width = 7, echo=FALSE}
ggplot(data=Icecream1, aes(x=time, y=amount)) + geom_point() + ggtitle("Icecream Dispensed") + xlab("Time Pressing Dispensor") + ylab("Amount Dispensed") + stat_smooth(method="lm", se=FALSE)
```

The blue line represents the least squares regression line.    

### Estimation in Ice Cream Example
...
### Ice Cream Model: Estimated Regression Equation

* The estimated regression equation is $E(Y_i) = `r IC_Model$coef[1]` + `r IC_Model$coef[2]`\times X_i$    

* The estimate $b_0 = `r IC_Model$coef[1]`$ is close to the true value of $\beta_0 = 0$   
* The estimate $b_1 = `r IC_Model$coef[2]`$ is close to the true value of $\beta_1 = 2$. 
* The estimate $s = `r summary(IC_Model)$sigma`$ is close to the true value of $\sigma = 0.5$. 
...
### True and Estimated Regression Line

```{r, fig.height=4, fig.width = 7, echo=FALSE}
ICplot + stat_smooth(method = "lm", se=FALSE) + geom_abline(slope=2, intercept=0, color="red")
```

The red line represents the true expectation function $E(\text{Amount}) = 0 + 2\times\text{Time}$.   
The blue line represents the estimated regression line $\widehat{\text{Amount}} = `r IC_Model$coef[1]` + `r IC_Model$coef[2]`\times\text{Time}$ 
...
time <- c(1, 1.2, 1.5, 1.8, 2.1, 2.1, 2.3, 2.5, 2.6, 2.8, 2.9, 2.9, 3.1, 3.2, 3.6)
amount <- 2*time + rnorm(15, 0, 0.5)
Icecream2 <- data.frame(time, amount)
IC_Model <- lm(data=Icecream2, lm(amount~time))
ICplot <- ggplot(data=Icecream2, aes(x=time, y=amount)) + geom_point() + ggtitle("Icecream Dispensed") + xlab("Time Pressing Dispensor") + ylab("Amount Dispensed") + geom_abline(slope=2, intercept=0, color="red")
ICplot + stat_smooth(method="lm", se=FALSE)
```

The red line represents the expectation function $E(Y_i) = 2X_i$.   

Estimated Regression Equation: $\widehat{\text{Amount}} = `r IC_Model$coef[1]` + `r IC_Model$coef[2]`\times \text{Time}$    

Estimate of $\sigma$:  `r summary(IC_Model)$sigma`
...
### Generate Many Samples from IC Example
...
### Distribution of $\frac{b_0 - \beta_0}{SE(b_0)}$ and $\frac{b_1 - \beta_1}{SE(b_1)}$ 
...
### Distribution of $\frac{{b_j}-\beta_j}{\text{SE}(b_j)}$  
...
## Tests and Intervals in Normal Error Regression Model
...
### Theory-Based CI for $\beta_0$ in Ice Cream Example

**95% confidence interval for $\beta_0$:**     

\[
b_0\pm t^*\text{SE}(b_0) = `r IC_Model$coef[1]` \pm `r 2` \times `r summary(IC_Model)$coeff[1,2]`
\]
...
We can be 95% confident that $\beta_0$ is between `r round(b0 - 2*SEb0,2)` and `r round(b0 + 2*SEb0,2)`. 


### Theory-Based CI for $\beta_1$ in Ice Cream Example

**95% confidence interval for $\beta_1$:**     

\[
b_1\pm 2\text{SE}(b_1) = `r IC_Model$coef[2]` \pm `r 2` \times `r summary(IC_Model)$coeff[2,2]`
\]
...
We can be 95% confident that $\beta_1$ is between `r round(b1 - 2*SEb1,2)` and `r round(b1 + 2*SEb1,2)`, meaning that we expect that the average amount of ice cream dispensed per second held is between `r round(b1 - 2*SEb1,2)` and `r round(b1 + 2*SEb1,2)` oz. 
...
### IC Example: Hypothesis Test for $\beta_0 = 0$

Null Hypothesis: $\beta_0=0$

Test Statistic: $t=\frac{b_0-0}{\text{SE}(b_0)}=\frac{`r IC_Model$coef[1]` - 0 }{`r summary(IC_Model)$coeff[1,2]`} = `r summary(IC_Model)$coeff[1,3]`$
...
gf_dist("t", df=13, geom = "area", fill = ~ (abs(x)< abs(ts)), show.legend=FALSE) + geom_vline(xintercept=c(ts, -ts), color="red")  + xlab("t")
```
...
### IC Example: Hypothesis Test for $\beta_1 = 0$

Null Hypothesis: $\beta_1=0$

Test Statistic: $t=\frac{b_1-0}{\text{SE}(b_1)}=\frac{`r IC_Model$coef[2]` - 0 }{`r summary(IC_Model)$coeff[2,2]`} = `r summary(IC_Model)$coeff[2,3]`$
...
```{r, echo=FALSE, fig.height=1.25, fig.width=6}
gf_dist("t", df=13, geom = "area", fill = "turquoise", show.legend=FALSE) + geom_vline(xintercept=c(ts, -ts), color="red")  + xlab("t")
```
...
### t-statistics and p-values in R
...
P3 <- ggplot(data=Cars2015, aes(sample = Cars_M_Log$residuals)) + stat_qq() + stat_qq_line() + xlab("Normal Quantiles") + ylab("Residual Quantiles") + ggtitle("Cars Model QQ Plot")
grid.arrange(P1, P2, P3, ncol=3)
```
...
P3 <- ggplot(data=data.frame(Cars_M7$residuals), aes(sample = Cars_M7$residuals)) + stat_qq() + stat_qq_line() + xlab("Normal Quantiles") + ylab("Residual Quantiles") + ggtitle("QQ Plot")
grid.arrange(P1, P2, P3, ncol=3)
```
...
P3 <- ggplot(data=data.frame(Bears_M2_MLR$residuals), aes(sample = Bears_M2_MLR$residuals)) + stat_qq() + stat_qq_line() + xlab("Normal Quantiles") + ylab("Residual Quantiles") + ggtitle("QQ Plot")
grid.arrange(P1, P2, P3, ncol=3)
```
...
P3 <- ggplot(data=data.frame(SAT_M3$residuals), aes(sample = SAT_M3$residuals)) + stat_qq() + stat_qq_line() + xlab("Normal Quantiles") + ylab("Residual Quantiles") + ggtitle("QQ Plot")
grid.arrange(P1, P2, P3, ncol=3)
```
...
P3 <- ggplot(data=data.frame(SAT_M4$residuals), aes(sample = SAT_M4$residuals)) + stat_qq() + stat_qq_line() + xlab("Normal Quantiles") + ylab("Residual Quantiles") + ggtitle("QQ Plot")
grid.arrange(P1, P2, P3, ncol=3)
```
